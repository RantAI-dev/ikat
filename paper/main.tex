...
showed a vision stage can substitute. Caveats: one judge model, text
stand-ins rather than pixels, and the judge shares a vendor with none of the
systems under test but with the evaluation tooling's author; blinding is the
mitigation, and per-item verdicts ship with the repo. Generator details and
protocol deviations (fp8 quantisation, 4096-token window with whole-chunk
truncation) are in the released runner.
