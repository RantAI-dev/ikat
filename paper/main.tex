...
\emph{requires a stage which looks at the image} is supported only by the
human-gold comparison: the MRAMG configuration contains no vision stage, so it
cannot speak to that half. Nor could a caption-rich corpus test it --- the claim
is about what happens when text about an image is absent or useless, which is the
regime the textbook corpus is in and MRAMG is not. That half therefore rests
entirely on 48 items annotated by one person. C1 is the weakest of the four: on questions answerable only from a figure,
