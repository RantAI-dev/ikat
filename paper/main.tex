...
\textbf{Scaling the sample is compute-bound, not annotation-bound.} A judge
cannot simply replace the annotator --- that is the circularity below --- but a
rectified estimator can use judge labels while subtracting the judge's bias,
measured on the human-labelled subset, in the manner of prediction-powered
inference~\cite{ares}. If the judge were uninformative such an estimator degrades
to the human-only estimate rather than to a wrong one. We implemented it and it is
not yet applicable to our data: the pairs carrying both labels came from two
annotation passes with different candidate pools, and because every gold figure
appears in both pools by construction, that intersection is enriched 2.46$\times$
in positives (12.1\,\% against 4.9\,\%) and is therefore not the random subsample
the correction requires. Making it applicable needs one judge pass over pools
drawn as the annotator's were --- inference, not annotation. We report the
implementation and the blocked gate rather than the estimate.
