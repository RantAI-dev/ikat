...
name. What this table does not measure is the axis the paper is about ---
\emph{where} an image lands and whether its placement is traceable to the
source. Arm A's insertions carry no provenance; the benchmark scores that
axis only through LLM-judged position metrics, which we defer rather than
proxy. Generator details and protocol deviations (fp8 quantisation, 4096-token
window with whole-chunk truncation) are in the released runner.
